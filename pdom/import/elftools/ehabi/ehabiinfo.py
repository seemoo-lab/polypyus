...
class EHABIInfo(object):
    ...
    def get_entry(self, n):
        ...
        eh_index_data = struct_parse(
            self._struct.EH_index_struct,
            self._arm_idx_section.stream,
            eh_index_entry_offset,
        )
        word0, word1 = eh_index_data["word0"], eh_index_data["word1"]

        if word0 & 0x80000000 != 0:
            return CorruptEHABIEntry(
                "Corrupt ARM exception handler table entry: %x" % n
            )

        function_offset = arm_expand_prel31(
            word0, self.section_offset() + n * EHABI_INDEX_ENTRY_SIZE
        )
        ...
        if word1 == 1:
            ...
            eh_table_offset = arm_expand_prel31(
                word1, self.section_offset() + n * EHABI_INDEX_ENTRY_SIZE + 4
            )
            eh_index_data = struct_parse(
                self._struct.EH_table_struct,
                self._arm_idx_section.stream,
                eh_table_offset,
            )
            ...
            if word0 & 0x80000000 == 0:
                ...
                return GenericEHABIEntry(
                    function_offset, arm_expand_prel31(word0, eh_table_offset)
                )
            else:
                ...
                if word0 & 0x70000000 != 0:
                    return CorruptEHABIEntry(
                        "Corrupt ARM compact model table entry: %x" % n
                    )
                ...
                if per_index == 0:
                    ...
                    return EHABIEntry(
                        ...
                        per_index,
                        opcode,
                        eh_table_offset=eh_table_offset,
                    )
                else:
                    return CorruptEHABIEntry(
...
class EHABIEntry(object):
    ...
    def __repr__(self):
        return "<EHABIEntry function_offset=0x%x, personality=%d, %sbytecode=%s>" % (
            ...
            self.personality,
            "eh_table_offset=0x%x, " % self.eh_table_offset
            if self.eh_table_offset
            else "",
            self.bytecode_array,
        )
...
class CorruptEHABIEntry(EHABIEntry):
    ...
    def __init__(self, reason):
        super(CorruptEHABIEntry, self).__init__(
            function_offset=None, personality=None, bytecode_array=None, corrupt=True
        )
        self.reason = reason
...
class CannotUnwindEHABIEntry(EHABIEntry):
    ...
    def __init__(self, function_offset):
        super(CannotUnwindEHABIEntry, self).__init__(
            function_offset, personality=None, bytecode_array=None, unwindable=False
        )
...
class GenericEHABIEntry(EHABIEntry):
    ...
    def __init__(self, function_offset, personality):
        super(GenericEHABIEntry, self).__init__(
            function_offset, personality, bytecode_array=None
        )

    def __repr__(self):
        return "<GenericEHABIEntry function_offset=0x%x, personality=0x%x>" % (
            self.function_offset,
            self.personality,
        )
