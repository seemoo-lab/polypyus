...
class Range(Subconstruct):
    ...
    def _parse(self, stream, context):
        ...
        try:
            ...
            if c < self.mincount:
                raise RangeError(
                    "expected %d to %d, found %d" % (self.mincount, self.maxcout, c), ex
                )
            stream.seek(pos)
        return obj
    ...
    def _build(self, obj, stream, context):
        if len(obj) < self.mincount or len(obj) > self.maxcout:
            raise RangeError(
                "expected %d to %d, found %d" % (self.mincount, self.maxcout, len(obj))
            )
        cnt = 0
        try:
            if self.subcon.conflags & self.FLAG_COPY_CONTEXT:
...
class Select(Construct):
    ...
    * include_name - a keyword only argument, indicating whether to include
    ...
    def __init__(self, name, *subcons, **kw):
        ...
        if kw:
            raise TypeError(
                "the only keyword argument accepted " "is 'include_name'", kw
            )
        Construct.__init__(self, name)
        self.subcons = subcons
