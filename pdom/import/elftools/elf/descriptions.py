...
def describe_rh_flags(x):
    return " ".join(
        ...
        for flag in (
            RH_FLAGS.RHF_NONE,
            RH_FLAGS.RHF_QUICKSTART,
            RH_FLAGS.RHF_NOTPOT,
            RH_FLAGS.RHF_NO_LIBRARY_REPLACEMENT,
            RH_FLAGS.RHF_NO_MOVE,
            RH_FLAGS.RHF_SGI_ONLY,
            RH_FLAGS.RHF_GUARANTEE_INIT,
            RH_FLAGS.RHF_DELTA_C_PLUS_PLUS,
            RH_FLAGS.RHF_GUARANTEE_START_INIT,
            RH_FLAGS.RHF_PIXIE,
            RH_FLAGS.RHF_DEFAULT_DELAY_LOAD,
            RH_FLAGS.RHF_REQUICKSTART,
            RH_FLAGS.RHF_REQUICKSTARTED,
            RH_FLAGS.RHF_CORD,
            RH_FLAGS.RHF_NO_UNRES_UNDEF,
...
def describe_sh_flags(x):
    ...
    for flag in (
        ...
        SH_FLAGS.SHF_LINK_ORDER,
        SH_FLAGS.SHF_OS_NONCONFORMING,
        SH_FLAGS.SHF_GROUP,
        SH_FLAGS.SHF_TLS,
        SH_FLAGS.SHF_MASKOS,
...
def describe_dt_flags(x):
    return " ".join(
        key[3:]
        for key, val in sorted(ENUM_DT_FLAGS.items(), key=lambda t: t[1])
        if x & val
    )
...
def describe_dt_flags_1(x):
    return " ".join(
        key[5:]
        for key, val in sorted(ENUM_DT_FLAGS_1.items(), key=lambda t: t[1])
        if x & val
    )
...
def describe_note(x):
    ...
    if x["n_type"] == "NT_GNU_ABI_TAG":
        if x["n_name"] == "Android":
            desc = "\n   description data: %s " % " ".join(
                "%02x" % ord(b) for b in x["n_descdata"]
            )
        else:
            desc = "\n    OS: %s, ABI: %d.%d.%d" % (
        ...
        desc = "\n    description data: {}".format(
            " ".join("{:02x}".format(ord(byte)) for byte in n_desc)
        )
    ...
    if x["n_type"] == "NT_GNU_ABI_TAG" and x["n_name"] == "Android":
        ...
        note_type = (
            x["n_type"] if isinstance(x["n_type"], str) else "Unknown note type:"
        )
        note_type_desc = (
            "0x%.8x" % x["n_type"]
            if isinstance(x["n_type"], int)
            else _DESCR_NOTE_N_TYPE.get(x["n_type"], _unknown)
        )
