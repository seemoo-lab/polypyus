...
class Section(object):
    ...
    def __init__(self, header, name, elffile):
        ...
        if self.compressed:
            ...
            header = struct_parse(
                self.structs.Elf_Chdr, self.stream, stream_pos=self["sh_offset"]
            )
...
class SymbolTableSection(Section):
    ...
    def __init__(self, header, name, elffile, stringtable):
        ...
        elf_assert(
            self["sh_entsize"] > 0, "Expected entry size of section %r to be > 0" % name
        )
        elf_assert(
            self["sh_size"] % self["sh_entsize"] == 0,
...
class SUNWSyminfoTableSection(Section):
    ...
    def get_symbol(self, n):
        ...
        entry_offset = self["sh_offset"] + n * self["sh_entsize"]
        entry = struct_parse(
            self.structs.Elf_Sunw_Syminfo, self.stream, stream_pos=entry_offset
        )
...
class ARMAttribute(object):
    ...
    def __init__(self, structs, stream):
        ...
        if self.tag in ("TAG_FILE", "TAG_SECTION", "TAG_SYMBOL"):
            ...
            self.value = struct_parse(
                structs.Elf_ntbs("value", encoding="utf-8"), stream
            )

        elif self.tag == "TAG_COMPATIBILITY":
            self.value = struct_parse(structs.Elf_uleb128("value"), stream)
            self.extra = struct_parse(
                structs.Elf_ntbs("vendor_name", encoding="utf-8"), stream
            )
            ...
            if type(self.value.value) is not str:
                ...
                elf_assert(
                    nul == 0, "Invalid terminating byte %r, expecting NUL." % nul
                )
...
class ARMAttributesSubsection(object):
    ...
    def __init__(self, stream, structs, offset):
        ...
        self.header = struct_parse(
            self.structs.Elf_Attr_Subsection_Header, self.stream, self.offset
        )
    ...
    def _make_subsubsections(self):
        ...
        while self.stream.tell() != end:
            subsubsec = ARMAttributesSubsubsection(
                self.stream, self.structs, self.stream.tell()
            )
...
class ARMAttributesSection(Section):
    ...
    def __init__(self, header, name, elffile):
        ...
        fv = struct_parse(
            self.structs.Elf_byte("format_version"), self.stream, self["sh_offset"]
        )

        elf_assert(
            chr(fv) == "A", "Unknown attributes version %s, expecting 'A'." % chr(fv)
        )
    ...
    def _make_subsections(self):
        ...
        while self.stream.tell() != end:
            subsec = ARMAttributesSubsection(
                self.stream, self.structs, self.stream.tell()
            )
