...
class ELFHashTable(object):
    ...
    def __init__(self, elffile, start_offset, symboltable):
        ...
        self.params = struct_parse(
            self.elffile.structs.Elf_Hash, self.elffile.stream, start_offset
        )
...
class GNUHashTable(object):
    ...
    def __init__(self, elffile, start_offset, symboltable):
        ...
        self._symboltable = symboltable
        self.params = struct_parse(
            self.elffile.structs.Gnu_Hash, self.elffile.stream, start_offset
        )
    ...
    def get_number_of_symbols(self):
        ...
        max_chain_pos = (
            self._chain_pos + (max_idx - self.params["symoffset"]) * self._wordsize
        )
        ...
        while True:
            cur_hash = struct.unpack(
                hash_format, self.elffile.stream.read(self._wordsize)
            )[0]
            if cur_hash & 1:
                return max_idx + 1
    ...
    def get_symbol(self, name):
        ...
        self.elffile.stream.seek(
            self._chain_pos + (symidx - self.params["symoffset"]) * self._wordsize
        )
        hash_format = "<I" if self.elffile.little_endian else ">I"
        while True:
            cur_hash = struct.unpack(
