...
class DynamicTag(object):
    ...
    _HANDLED_TAGS = frozenset(
        ["DT_NEEDED", "DT_RPATH", "DT_RUNPATH", "DT_SONAME", "DT_SUNW_FILTER"]
    )
    ...
    def __init__(self, entry, stringtable):
        ...
        if entry.d_tag in self._HANDLED_TAGS:
            setattr(
                self, entry.d_tag[3:].lower(), stringtable.get_string(self.entry.d_val)
            )
...
class DynamicSegment(Segment, Dynamic):
    ...
    def __init__(self, header, stream, elffile):
        ...
        for section in elffile.iter_sections():
            if (
                isinstance(section, DynamicSection)
                and section["sh_offset"] == header["p_offset"]
            ):
                stringtable = elffile.get_section(section["sh_link"])
                break
        Segment.__init__(self, header, stream)
        Dynamic.__init__(
            self, stream, elffile, stringtable, self["p_offset"], self["p_filesz"] == 0
        )
    ...
    def iter_symbols(self):
        ...
        if gnu_hash_offset is not None:
            ...
            end_ptr = tab_ptr + hash_section.get_number_of_symbols() * symbol_size
        ...
        if end_ptr is None:
            ...
            if nearest_ptr is None:
                ...
                for segment in self.elffile.iter_segments():
                    if segment["p_vaddr"] <= tab_ptr and tab_ptr <= (
        ...
        for i in range((end_ptr - tab_ptr) // symbol_size):
            symbol = struct_parse(
                self.elfstructs.Elf_Sym, self._stream, i * symbol_size + tab_offset
            )
