...
from .gnuversions import GNUVerNeedSection, GNUVerDefSection, GNUVerSymSection
...
class ELFFile(object):
    ...
    def __init__(self, stream):
        ...
        self.structs = ELFStructs(
            little_endian=self.little_endian, elfclass=self.elfclass
        )

        self.structs.create_basic_structs()
        self.header = self._parse_elf_header()
        self.structs.create_advanced_structs(
            self["e_type"], self["e_machine"], self["e_ident"]["EI_OSABI"]
        )
        self.stream.seek(0)
    ...
    def has_dwarf_info(self):
        ...
        """
        return bool(
            self.get_section_by_name(".debug_info")
            or self.get_section_by_name(".zdebug_info")
            or self.get_section_by_name(".eh_frame")
        )
        ...
        for secname in section_names:
            ...
            if section is None:
                ...
                dwarf_section = self._read_dwarf_section(
                    section, relocate_dwarf_sections
                )
    ...
    def _get_section_header(self, n):
        ...
        return struct_parse(
            self.structs.Elf_Shdr, self.stream, stream_pos=self._section_offset(n)
        )
    ...
    def _make_symbol_table_section(self, section_header, name):
        ...
        return SymbolTableSection(
            section_header, name, elffile=self, stringtable=strtab_section
        )
    ...
    def _make_symbol_table_index_section(self, section_header, name):
        ...
        return SymbolTableIndexSection(
            section_header, name, elffile=self, symboltable=linked_symtab_index
        )
    ...
    def _make_sunwsyminfo_table_section(self, section_header, name):
        ...
        return SUNWSyminfoTableSection(
            section_header, name, elffile=self, symboltable=strtab_section
        )
    ...
    def _get_segment_header(self, n):
        ...
        return struct_parse(
            self.structs.Elf_Phdr, self.stream, stream_pos=self._segment_offset(n)
        )
    ...
    def _decompress_dwarf_section(section):
        ...
        assert compression_type == b"ZLIB", "Invalid compression type: %r" % (
            compression_type
        )
