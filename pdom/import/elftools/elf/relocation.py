...
class RelocationSection(Section, RelocationTable):
    ...
    def __init__(self, header, name, elffile):
        ...
        elf_assert(
            header["sh_type"] in ("SHT_REL", "SHT_RELA"),
            "Unknown relocation type section",
        )
        elf_assert(
            header["sh_entsize"] == self.entry_size,
...
class RelocationHandler(object):
    ...
    def find_relocations_for_section(self, section):
        ...
        for relsection in self.elffile.iter_sections():
            if (
                isinstance(relsection, RelocationSection)
                and relsection.name in reloc_section_names
            ):
                return relsection
        return None
    ...
    def _do_apply_relocation(self, stream, reloc, symtab):
        ...
        if recipe.bytesize == 4:
            ...
            raise ELFRelocationError(
                "Invalid bytesize %s for relocation" % recipe.bytesize
            )

        # 1. Read the value from the stream (with correct size and endianness)
        original_value = struct_parse(
            value_struct, stream, stream_pos=reloc["r_offset"]
        )
    ...
    #
    _RELOCATION_RECIPE_TYPE = namedtuple(
        "_RELOCATION_RECIPE_TYPE", "bytesize has_addend calc_func"
    )
    ...
    _RELOCATION_RECIPES_ARM = {
        ...
        ENUM_RELOC_TYPE_ARM["R_ARM_CALL"]: _RELOCATION_RECIPE_TYPE(
            bytesize=4, has_addend=False, calc_func=_arm_reloc_calc_sym_plus_value_pcrel
        ),
    }
