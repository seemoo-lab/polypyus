...
class Segment(object):
    ...
    def section_in_segment(self, section):
        ...
        # sections
        if secflags & SH_FLAGS.SHF_TLS and segtype in (
            "PT_TLS",
            "PT_GNU_RELRO",
            "PT_LOAD",
        ):
        ...
        # at all
        elif (secflags & SH_FLAGS.SHF_TLS) == 0 and segtype not in (
            "PT_TLS",
            "PT_PHDR",
        ):
            pass
        ...
        if (secflags & SH_FLAGS.SHF_ALLOC) == 0 and segtype in (
            "PT_LOAD",
            "PT_DYNAMIC",
            "PT_GNU_EH_FRAME",
            "PT_GNU_RELRO",
        ...
        if secflags & SH_FLAGS.SHF_ALLOC:
            ...
            if not (
                secaddr >= vaddr
                and secaddr - vaddr + section["sh_size"] <= self["p_memsz"]
                and secaddr - vaddr <= self["p_memsz"] - 1
            ):
                return False
        ...
        return (
            secoffset >= poffset
            and secoffset - poffset + section["sh_size"] <= self["p_filesz"]
            and secoffset - poffset <= self["p_filesz"] - 1
        )
...
class InterpSegment(Segment):
    ...
    def get_interp_name(self):
        ...
        return struct_parse(
            CString("", encoding="utf-8"), self.stream, stream_pos=path_offset
        )
