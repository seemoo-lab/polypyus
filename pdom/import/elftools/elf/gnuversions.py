...
class GNUVersionSection(Section):
    ...
    def _iter_version_auxiliaries(self, entry_offset, count):
        ...
        for _ in range(count):
            entry = struct_parse(
                self.version_auxiliaries_struct, self.stream, stream_pos=entry_offset
            )
    ...
    def iter_versions(self):
        ...
        for _ in range(self.num_versions()):
            entry = struct_parse(
                self.version_struct, self.stream, stream_pos=entry_offset
            )

            elf_assert(
                entry[count_field] > 0,
                "Expected number of version auxiliary entries (%s) to be > 0"
                "for the following version entry: %s" % (count_field, str(entry)),
            )
...
class GNUVerNeedSection(GNUVersionSection):
    ...
    def __init__(self, header, name, elffile, stringtable):
        super(GNUVerNeedSection, self).__init__(
            ...
            stringtable,
            "vn",
            elffile.structs.Elf_Verneed,
            elffile.structs.Elf_Vernaux,
        )
...
class GNUVerDefSection(GNUVersionSection):
    ...
    def __init__(self, header, name, elffile, stringtable):
        super(GNUVerDefSection, self).__init__(
            ...
            stringtable,
            "vd",
            elffile.structs.Elf_Verdef,
            elffile.structs.Elf_Verdaux,
        )
...
class GNUVerSymSection(Section):
    ...
    def get_symbol(self, n):
        ...
        entry = struct_parse(
            self.structs.Elf_Versym, self.stream, stream_pos=entry_offset
        )
