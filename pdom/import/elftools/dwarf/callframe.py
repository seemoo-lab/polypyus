...
class CallFrameInfo(object):
    ...
    def _parse_entry_at(self, offset):
        ...
        if self.for_eh_frame:
            ...
            is_CIE = (
                dwarf_format == 32 and CIE_id == 0xFFFFFFFF
            ) or CIE_id == 0xFFFFFFFFFFFFFFFF

        # Parse the header, which goes up to and excluding the sequence of
        # instructions.
        if is_CIE:
            header_struct = (
                entry_structs.EH_CIE_header
                if self.for_eh_frame
                else entry_structs.Dwarf_CIE_header
            )
        ...
        if is_CIE:
            ...
            lsda_encoding = cie.augmentation_dict.get(
                "LSDA_encoding", DW_EH_encoding_flags["DW_EH_PE_omit"]
            )
            if lsda_encoding != DW_EH_encoding_flags["DW_EH_PE_omit"]:
                # parse LSDA pointer
                lsda_pointer = self._parse_lsda_pointer(
                    entry_structs, self.stream.tell() - len(aug_bytes), lsda_encoding
                )
            else:
                lsda_pointer = None
    ...
    def _parse_instructions(self, structs, offset, end_offset):
        ...
        while offset < end_offset:
            ...
            # primary == 0 and real opcode is extended
            elif opcode in (DW_CFA_nop, DW_CFA_remember_state, DW_CFA_restore_state):
                args = []
                ...
                args = [
                    struct_parse(structs.Dwarf_dw_form["DW_FORM_block"], self.stream)
                ]
    ...
    def _parse_cie_augmentation(self, header, entry_structs):
        ...
        assert augmentation.startswith(
            b"z"
        ...
        available_fields = {
            ...
            b"P": Struct(
                ...
                Switch(
                    "function",
                    lambda ctx: ctx.encoding & 0x0F,
                    {
                        enc: fld_cons("function")
                        for enc, fld_cons in self._eh_encoding_to_field(
                            entry_structs
                        ).items()
                    },
                ),
    ...
    def _read_augmentation_data(self, entry_structs):
        ...
        augmentation_data_length = struct_parse(
            Struct("Dummy_Augmentation_Data", entry_structs.Dwarf_uleb128("length")),
            self.stream,
        )["length"]
    ...
    def _parse_lsda_pointer(self, structs, stream_offset, encoding):
        ...
        ptr = struct_parse(
            Struct("Augmentation_Data", formats[basic_encoding]("LSDA_pointer")),
            self.stream,
            stream_pos=stream_offset,
        )["LSDA_pointer"]
        ...
        if modifier == DW_EH_encoding_flags["DW_EH_PE_absptr"]:
            ...
            assert (
                False
            ), "Unsupported encoding modifier for LSDA pointer: {:#x}".format(modifier)
    ...
    def _parse_fde_header(self, entry_structs, offset):
        ...
        fields = [
            entry_structs.Dwarf_initial_length("length"),
        ...
        minimal_header = struct_parse(
            Struct("eh_frame_minimal_header", *fields), self.stream, offset
        )
    ...
    def _decode_CFI_table(self):
        ...
        for instr in self.instructions:
            ...
            if name == "DW_CFA_set_loc":
                ...
                cur_line["cfa"] = CFARule(
                    reg=instr.args[0], offset=cur_line["cfa"].offset
                )
                ...
                cur_line[instr.args[0]] = RegisterRule(
                    RegisterRule.OFFSET, instr.args[1] * cie["data_alignment_factor"]
                )
                ...
                _add_to_order(instr.args[0])
                cur_line[instr.args[0]] = RegisterRule(
                    RegisterRule.REGISTER, instr.args[1]
                )
            elif name == "DW_CFA_expression":
                _add_to_order(instr.args[0])
                cur_line[instr.args[0]] = RegisterRule(
                    RegisterRule.EXPRESSION, instr.args[1]
                )
            elif name == "DW_CFA_val_expression":
                _add_to_order(instr.args[0])
                cur_line[instr.args[0]] = RegisterRule(
                    RegisterRule.VAL_EXPRESSION, instr.args[1]
                )
            elif name in ("DW_CFA_restore", "DW_CFA_restore_extended"):
                _add_to_order(instr.args[0])
                dwarf_assert(
                    isinstance(self, FDE), "%s instruction must be in a FDE" % name
                )
...
# their appearance.
#
DecodedCallFrameTable = namedtuple("DecodedCallFrameTable", "table reg_order")
