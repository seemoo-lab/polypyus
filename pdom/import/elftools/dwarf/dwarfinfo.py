...
from ..common.utils import struct_parse, dwarf_assert, parse_cstring_from_stream
...
#
DebugSectionDescriptor = namedtuple(
    "DebugSectionDescriptor", "stream name global_offset size address"
)
...
#
DwarfConfig = namedtuple(
    "DwarfConfig", "little_endian machine_arch default_address_size"
)
...
class DWARFInfo(object):
    ...
    def get_abbrev_table(self, offset):
        ...
        if offset not in self._abbrevtable_cache:
            self._abbrevtable_cache[offset] = AbbrevTable(
                structs=self.structs, stream=self.debug_abbrev_sec.stream, offset=offset
            )
    ...
    def get_pubtypes(self):
        ...
        if self.debug_pubtypes_sec:
            return NameLUT(
                self.debug_pubtypes_sec.stream,
    ...
    def get_pubnames(self):
        ...
        if self.debug_pubnames_sec:
            return NameLUT(
                self.debug_pubnames_sec.stream,
    ...
    def get_aranges(self):
        ...
        if self.debug_aranges_sec:
            return ARanges(
                self.debug_aranges_sec.stream, self.debug_aranges_sec.size, self.structs
            )
        else:
            return None
    ...
    def _parse_line_program_at_offset(self, debug_line_offset, structs):
        ...
        lineprog_header = struct_parse(
            structs.Dwarf_lineprog_header, self.debug_line_sec.stream, debug_line_offset
        )

        # Calculate the offset to the next line program (see DWARF 6.2.4)
        end_offset = (
            debug_line_offset
            + lineprog_header["unit_length"]
