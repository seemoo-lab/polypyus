...
def describe_CFI_instructions(entry):
    ...
    def _assert_FDE_instruction(instr):
        dwarf_assert(
            isinstance(entry, FDE), 'Unexpected instruction "%s" for a CIE' % instr
        )
    ...
    for instr in entry.instructions:
        ...
        if name in (
            ...
            pc += factored_offset
        elif name in ("DW_CFA_remember_state", "DW_CFA_restore_state", "DW_CFA_nop"):
            s += "  %s\n" % name
...
_EXTRA_INFO_DESCRIPTION_MAP = defaultdict(
    ...
    DW_AT_accessibility=_make_extra_mapper(_DESCR_DW_ACCESS, "(unknown accessibility)"),
    DW_AT_visibility=_make_extra_mapper(_DESCR_DW_VIS, "(unknown visibility)"),
    DW_AT_virtuality=_make_extra_mapper(_DESCR_DW_VIRTUALITY, "(unknown virtuality)"),
    DW_AT_identifier_case=_make_extra_mapper(_DESCR_DW_ID_CASE, "(unknown case)"),
    DW_AT_calling_convention=_make_extra_mapper(_DESCR_DW_CC, "(unknown convention)"),
    DW_AT_ordering=_make_extra_mapper(_DESCR_DW_ORD, "(undefined)"),
...
class ExprDumper(object):
    ...
    def _dump_to_string(self, opcode, opcode_name, args, cu_offset=None):
        ...
        if len(args) == 0:
            ...
            return "%s: <0x%x>  %d byte block: %s " % (
                opcode_name,
                args[0] + cu_offset,
                len(args[1]),
                " ".join("%x" % b for b in args[1]),
            )
