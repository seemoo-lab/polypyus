...
class DWARFStructs(object):
        ...
        def _InitialLength(name):
            ...
            return _InitialLengthAdapter(
                Struct(
                    ...
                    If(
                        lambda ctx: ctx.first == 0xFFFFFFFF,
    ...
    def _create_abbrev_declaration(self):
        self.Dwarf_abbrev_declaration = Struct(
            "Dwarf_abbrev_entry",
            Enum(self.Dwarf_uleb128("tag"), **ENUM_DW_TAG),
            Enum(self.Dwarf_uint8("children_flag"), **ENUM_DW_CHILDREN),
            RepeatUntilExcluding(
                lambda obj, ctx: obj.name == "DW_AT_null"
                and obj.form == "DW_FORM_null",
                Struct(
                    "attr_spec",
    ...
    def _create_dw_form(self):
        self.Dwarf_dw_form = dict(
            ...
            DW_FORM_ref_addr=self.Dwarf_target_addr("")
            if self.dwarf_version == 2
            else self.Dwarf_offset(""),
    ...
    def _create_lineprog_header(self):
        ...
        self.Dwarf_lineprog_file_entry = Struct(
            "file_entry",
            CString("name"),
            If(
                lambda ctx: len(ctx.name) != 0,
                Embed(
                    Struct(
                        "",
        ...
        self.Dwarf_lineprog_header = Struct(
            ...
            Array(
                lambda ctx: ctx["opcode_base"] - 1,
                self.Dwarf_uint8("standard_opcode_lengths"),
            ),
            RepeatUntilExcluding(
                lambda obj, ctx: obj == b"", CString("include_directory")
            ),
            RepeatUntilExcluding(
                lambda obj, ctx: len(obj.name) == 0, self.Dwarf_lineprog_file_entry
            ),
        )
    ...
    def _make_block_struct(self, length_field):
        ...
        return PrefixedArray(
            subcon=self.Dwarf_uint8("elem"), length_field=length_field("")
        )
        ...
        if obj.first < 0xFFFFFF00:
            ...
            if obj.first == 0xFFFFFFFF:
                ...
                raise ConstructError(
                    "Failed decoding initial length for %X" % (obj.first)
                )
