...
#
AttributeValue = namedtuple("AttributeValue", "name form value raw_value offset")
...
class DIE(object):
    ...
    def _search_ancestor_offspring(self):
        ...
        while search.offset < self.offset:
            ...
            if prev is search:
                raise ValueError(
                    "offset %s not in CU %s DIE tree" % (self.offset, self.cu.cu_offset)
                )
    ...
    def _translate_attr_value(self, form, raw_value):
        ...
        if form == "DW_FORM_strp":
            ...
            try:
                ...
                raise DWARFError(
                    "Found DW_FORM_indirect with unknown raw_value=" + str(raw_value)
                )
