...
LocationEntry = namedtuple(
    "LocationEntry", "entry_offset begin_offset end_offset loc_expr"
)
    ...
    def _parse_location_list_from_stream(self):
        ...
        while True:
            ...
            if begin_offset == 0 and end_offset == 0:
                ...
                lst.append(
                    BaseAddressEntry(entry_offset=entry_offset, base_address=end_offset)
                )
            else:
                # Location list entry
                expr_len = struct_parse(self.structs.Dwarf_uint16(""), self.stream)
                loc_expr = [
                    struct_parse(self.structs.Dwarf_uint8(""), self.stream)
...
class LocationParser(object):
    ...
    def attribute_has_location(attr, dwarf_version):
        ...
        return LocationParser._attribute_is_loclistptr_class(attr) and (
            LocationParser._attribute_has_loc_expr(attr, dwarf_version)
    ...
    def parse_from_attribute(self, attr, dwarf_version):
        ...
        if self.attribute_has_location(attr, dwarf_version):
            if self._attribute_has_loc_expr(attr, dwarf_version):
                ...
                return self.location_lists.get_location_list_at_offset(attr.value)
        else:
    ...
    def _attribute_has_loc_expr(attr, dwarf_version):
        return (
            ...
            and not attr.name == "DW_AT_const_value"
