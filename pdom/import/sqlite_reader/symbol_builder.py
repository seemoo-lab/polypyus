...
class SymbolBuilder:
    ...
    def build_function(self, line):
        ...
        if functionType != None:
            ...
            for parameter_index in range(parameter_count):
                ...
                if (
                    parameter_indentifier == " "
                    or parameter_indentifier == ""
                    or parameter_indentifier == None
                ):
                    parameter_indentifier = "UNKNOWN"
    ...
    def build_struct(self, line):
        if not self.createCSV:
            ...
            for field in fields:
                try:
                    if (
                        field[1].get("table") == "PDOMCPPConstructor"
                        or field[1].get("table") == "PDOMCPPMethod"
                        or field[1].get("table") == "PDOMCPPFunction"
                        or field[1].get("table") == "PDOMCFunction"
                    ):
                        ...
                        IDAtools.set_struct_member(
                            structname, fieldName, flag_andTypeID, size
                        )
                    else:
                        fieldName = field[1].get("name")
                        ...
                        if type:
                            if (
                                type.get("table") == "BasicType"
                                and field[1].get("table") == "PDOMCField"
                            ):
        ...
        if not self.createCSV:
            ...
            field_membrs = [
                0,
                {
                    "table": "PDOMCField",
                    "name": "__mbrs",
                    "type": {"table": "PDOMCPPTypedef", "name": name + "_mbrs"},
                },
            ]
            field_vtbl = [
                1,
    ...
    def get_type_string(self, type_dict):
        ...
        if type_dict == None:
            return "_UNKNOWN"
        try:
            while type_dict.get("table") != "BasicType" and not (
                type_dict.get("table").startswith("PDOM")
            ):
