...
class ReadDatabase:
	...
	def import_symbols_from_list(sql_controller, gui_Object, symbol_name_list, symbol_builder, full_search=False):	
		...
		for lines in symbol_name_list:	
			...
			if len(symbol_name_list) > 2:
				gui_Object.set_progress(int(progressCounter/len(symbol_name_list)*100))		
			...
			if lines != "":
				...
				result = sql_controller.get_entry_by_name("PDOMCPPMethodSpecialization", lines)
				if result:
				...
				result = sql_controller.get_entry_by_name("PDOMCPPConstructorTemplate", lines)
				if result:
					cpp_reader.read_cpp_constructor_template(result)
					continue
				
				result = sql_controller.get_entry_by_name("PDOMCPPFunctionInstance", lines)
				if result:
					cpp_reader.read_cpp_function_instance(result)
					continue
				
				result = sql_controller.get_entry_by_name("PDOMCPPFunctionTemplate", lines)
				if result:
					cpp_reader.read_cpp_function_template(result)
					continue

				result = sql_controller.get_entry_by_name("PDOMCPPMethodInstance", lines)
				if result:
					cpp_reader.read_cpp_method_instance(result)
					continue
				
				result = sql_controller.get_entry_by_name("PDOMCPPMethodTemplate", lines)
				if result:
					cpp_reader.read_cpp_method_template(result)
					continue
				
				result = sql_controller.get_entry_by_name("PDOMCPPMethodTemplateSpecialization", lines)
				if result:
				...
				result = sql_controller.get_entry_by_name("PDOMCPPClassSpecialization", lines)
				if result:
				...
				result = sql_controller.get_entry_by_name("PDOMCPPClassTemplatePartialSpecialization", lines)
				if result:
				...
				result = sql_controller.get_entry_by_name("PDOMCPPTypedefSpecialization", lines)
				if result:
