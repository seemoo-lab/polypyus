...
class Controller:
	...
	def import_file(gui_Object,filePath,import_functions,import_objects,import_sections,overwrite_names,offset):
		progressCounter = 0
		...
		if filePath.endswith(".elf"): #elf file
			...
			for symbol in listSymbols:
				...
				Controller.create_function_data(symbol,overwrite_names,offset,'FUNC','OBJECT',import_functions,import_objects)
			...
			for i in fileLines:
				...
				try:
					symbol = {'name' : line[0], 'addr' : str(int(line[1],16)),'size' : line[2],'type' : line[4]}
				except (IndexError,ValueError):
					continue
				Controller.create_function_data(symbol,overwrite_names,offset,'FUNC','OBJECT',import_functions,import_objects)

	def import_userdefined_csv(gui_Object,filePath,import_functions,import_objects,overwrite_names,offset,delimiter,identFunc,identData,nameCol,addrCol,typeCol,sizeCol):
		progressCounter = 0
		...
		for i in fileLines:
			...
			line = list(filter(None, line))
			try:
				symbol = {'name' : line[int(nameCol)], 'addr' : str(int(line[int(addrCol)],16)),'size' : line[int(sizeCol)],'type' : line[int(typeCol)]}
			except (IndexError,ValueError):
				continue

			Controller.create_function_data(symbol,overwrite_names,offset,identFunc,identData,import_functions,import_objects)


	def create_function_data(symbol, overwrite_names, offset, identFunc, identData,import_functions,import_objects):
...
######################################################################## Signature import ###############################################################################
	...
	def import_database(gui_Object, referenceFilePath, additionalPathList):
		...
		for database_path in additionalPathList:			
			gui_Object.set_state("Build CSV " + str(progress_counter) + "/" + str(len(additionalPathList)))
			...
			created_csv_path = csvFolder + database_path.split("/")[-1] + "." + str(time.time()) + ".csv"

			Controller = SqlController(database_path)

			#search for signatures and produce csv:
			listOfFunctioNames = IDAtools.get_names()
			ReadDatabase.import_symbols_from_list(Controller,gui_Object,listOfFunctioNames, symbol_builder)
		...
		finalSignaturesCSVPath = SignatureBuilder.get_final_signature_csv(csvFiles, referencePDOM,csvFolder)
		listImportSig = list(csv.reader(open(finalSignaturesCSVPath, 'r'), delimiter=";"))
		...
		for line in listImportSig: #[symbolName, signature, databasePDOM1, databasePDOM2, ....]
			...
			for pdoms in line[2:]:
				...
				ReadDatabase.import_symbols_from_list(Controller,gui_Object, [symbol], symbol_builder)
