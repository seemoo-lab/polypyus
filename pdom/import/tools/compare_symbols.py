...
class SignatureBuilder:
    def get_final_signature_csv(files, reference_PDOM, final_csv_path):
        ...
        for mappedSymbol in symbolDict:
            finalSymbolMap.append(
                SignatureBuilder.get_best_signature(
                    mappedSymbol, symbolDict[mappedSymbol], reference_PDOM
                )
            )
    ...
    def get_best_signature(symbol, signature_Dict, reference_PDOM):
        ...
        if len(signature_Dict) == 1:  # return if there is just one signature available
            for (
                ...
                signature_Dict
            ):  # do parse and build_parsed to correct buggy signatures
                return [
                    symbol,
                    SignatureBuilder.build_parsed_Function(
                        SignatureBuilder.parse_function(i)
                    ),
                    signature_Dict[i][0],
                ]
        ...
        ) = SignatureBuilder.get_possible_types(signature_Dict)

        # choose refernence signature
        reference_signature = SignatureBuilder.chosse_referenz_signature(
            signature_Dict, reference_PDOM
        )
        reference_signature_parsed = SignatureBuilder.parse_function(
            reference_signature
        )
        ...
        if (
            reference_signature_function_type == "_UNKNOWN"
            and possible_function_types_dict
        ):
            decided_function_type = max(
                possible_function_types_dict,
                key=lambda type_: possible_function_types_dict.get(type_).get("count"),
            )
            if decided_function_type == "":
                decided_function_type = "_UNKNOWN"
            else:
                usedPDOMS.append(
                    possible_function_types_dict[decided_function_type]["pdom"]
                )
        else:
        ...
        for reference_parameter in reference_signature_parsed.get("parameter"):
            ...
            if (
                current_reference_parameter_type == "_UNKNOWN"
                or current_reference_parameter_type == ""
            ):
                for types in possible_parameter_list[parameter_index][
                    current_reference_parameter_name
                ]:
                    if types != "_UNKNOWN" and types != "":
                        possible_parameter_list_known = {
                            key: value
                            for (key, value) in possible_parameter_list[
                                parameter_index
                            ][current_reference_parameter_name].items()
                            if not "_UNKNOWN" in key and key != ""
                        }

                if len(possible_parameter_list_known) == 0:
                    decidedParameter = "_UNKNOWN"
                else:
                    decidedParameter = max(
                        possible_parameter_list_known,
                        key=lambda x: possible_parameter_list_known[x].get("count"),
                    )
                    usedPDOMS.append(
                        possible_parameter_list_known.get(decidedParameter).get("pdom")
                    )
        ...
        signature = SignatureBuilder.build_parsed_function_from_decided_types(
            decided_function_type, reference_signature_parsed, decided_parameter_list
        )
    ...
    def chosse_referenz_signature(signature_Dict, reference_PDOM):
        ...
        return max(
            signature_Dict,
            key=lambda signature_: (
                len(SignatureBuilder.parse_function(signature_).get("parameter")),
                len(signature_Dict[signature_]),
            ),
        )
    ...
    def get_possible_types(signature_Dict):
        ...
        for signature in signature_Dict:
            ...
            for parameter_Index in range(len(current_parameter_list)):
                current_parameter_type = current_parameter_list[parameter_Index].get(
                    "type"
                )
                current_parameter_name = current_parameter_list[parameter_Index].get(
                    "name"
                )
                try:
                    if (
                        current_parameter_name
                        in possible_parameter_list[parameter_Index]
                    ):
                        if (
                            ...
                            possible_parameter_list[parameter_Index][
                                current_parameter_name
                            ][current_parameter_type]["count"] = (
                                possible_parameter_list[parameter_Index]
                                .get(current_parameter_name)
                                .get(current_parameter_type)
                                .get("count")
                                + 1
                            )
                        else:
                            possible_parameter_list[parameter_Index][
                                ...
                                "count": 1,
                                "pdom": signature_Dict[signature][0],
                            }
                    else:
                        possible_parameter_list[parameter_Index][
                            current_parameter_name
                    ...
                    possible_parameter_list.append(
                        {
                            current_parameter_name: {
                                current_parameter_type: {
                                    "count": 1,
            ...
            if (
                not (current_function_type in possible_function_types_dict)
                and current_function_type != "_UNKNOWN"
            ):
                possible_function_types_dict[current_function_type] = {
                    "count": 1,
                    "pdom": signature_Dict[signature][0],
                }
            elif current_function_type != "_UNKNOWN":
                possible_function_types_dict[current_function_type]["count"] = (
                    possible_function_types_dict.get(current_function_type).get("count")
                    + 1
                )
    ...
    def build_parsed_function_from_decided_types(
        decided_function_type, reference_signature_parsed, decided_parameter_list
    ):
        signature = (
            decided_function_type + " " + reference_signature_parsed.get("name") + "("
        )
        index = 0
        ...
        for decided_parameter_types in decided_parameter_list:
            parameterName = reference_signature_parsed.get("parameter")[index].get(
                "name"
            )
            if (
                decided_parameter_types == ""
                or decided_parameter_types == " "
                or decided_parameter_types is None
            ):
            ...
            if index == len(decided_parameter_list) - 1:
                signature = (
                    signature + decided_parameter_types + " " + parameterName + ")"
                )
            else:
                signature = (
                    signature + decided_parameter_types + " " + parameterName + ","
    ...
    def parse_function(functionString):
        ...
        functionDict = {}
        functionDict["parameter"] = [
            {"type": "_UNKNOWN", "name": "UNKNOWN"}
        ] * parameterCounter
        ...
        while index > -1:
            ...
            if tokens[index] == "," or tokens[index] == ")":
                ...
                functionDict["parameter"][parameter_index] = {
                    "type": currentType,
