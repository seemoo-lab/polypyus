    ...
    def get_register(file_path):
        ...
        for lines in list_regs:
            try:
                ...
                output_list.append(
                    {"name": name, "addr": addr, "size": "4", "type": typeString}
                )
            except Exception:
                pass
    ...
    def get_segments(file_path):
        ...
        for regs in register:
            ...
            for segment in segments:
                if (addr + tolerance - segment.get("start")) >= 0 and (
                    addr - tolerance - segment.get("end")
                ) <= 0:
            ...
            if not ok:
                segments.append(
                    {
                        "name": "hardware_register_segment_" + str(len(segments)),
                        "start": addr,
                        "end": addr + 4,
                        "R": 1,
                        "W": 1,
        ...
        minRegisterCount = 10
        segments = list(
            filter(lambda x: len(x.get("register")) > minRegisterCount, segments)
        )
