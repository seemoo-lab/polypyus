...
class IDAtools:
	...
	def is_dummy_name_by_addr(addr):
	    ...
	    dummyList = {"","sub_","locret_","loc_","off_","seg_","asc_","byte_","word_","dword_","qword_","byte3_","xmmword","_ymmword_","packreal_","flt_","dbl_","tbyte_","stru_","custdata_","algn_","unk_"}
	...
	def create_function(fnc, overwrite_names, offset, add_names=True,):
		...
		if type(addr) == int:
			...
			if IDAtools.is_dummy_name_by_addr(addr) and (not name.startswith("sub_") and overwrite_names):
				set_name(addr,name)
	...
	def create_segement(segm, offset):
		...
		if idaapi.is_seg(idc.get_inf_attr(idc.INF_MAX_EA),0):
			...
			ok = idaapi.add_segm(base, start, idc.get_inf_attr(idc.INF_MAX_EA), name, class_)
	...
	def create_object(obj, overwrite_names, offset,dummy_names=True, always_thumb=True):
		name = obj.get("name")
		...
		if overwrite_names and IDAtools.is_dummy_name_by_addr(addr):
			ok = idc.set_name(addr, name, idc.SN_CHECK)
			if not ok:
				reason = "Could not add name {name} at {addr:#x}".format(name=name, addr=addr)
				print(reason)
	...
	def set_enum_member(name, member, value):
		...
		if not ok:
			if idaapi.get_enum_member_by_name(str(member)) == idaapi.BADADDR:
				print("Could not add enum member {member} at {name}".format(name=name, member=member))
	...
	def set_function_type(name,TypeString, print_ERROR=False):
		...
		if not ok and print_ERROR:
			print("Could not add function type {name} at {ea:#x} : {TypeString}".format(name=name,ea=ea,TypeString=TypeString))	
	...
	def set_struct_member(structname,name, flag_andTypeID, size):
		...
		try:
			ok = 0
			if not idaapi.get_member_by_name(idaapi.get_struc(idaapi.get_struc_id(str(structname))) ,str(name)):
				ok = idc.add_struc_member(id, str(name), offset, flag , typeid, nbytes)
		
			if not ok:
				if not idaapi.get_member_by_name(idaapi.get_struc(idaapi.get_struc_id(str(structname))) ,str(name)):
					print("Could not add struct member {name} at {structname}".format(name=name,structname=structname))	
		except:
			pass
	...
	def set_global_data(name, TypeString):
		...
		ok = idc.SetType(ea,str(TypeString))
		if not ok:
			print("Could not add global type {name} at {ea:#x} : {TypeString}".format(name=name,ea=ea,TypeString=TypeString))		
	...
	def get_size(name, type, line):
		if type == "BasicType":
			...
			typeSize = IDAtools.get_size(str(subType.get("name")),subType.get("table"),subType)
			try:
