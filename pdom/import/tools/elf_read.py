...
class Elfloader:
			...
			if section.header.sh_type in ['SHT_SYMTAB', 'SHT_DYNSYM']:
				for symbol in section.iter_symbols():
					...
					if name != "" and "$" not in name:
						yield {"name": name, "value": value, "type": typ, "entry": entry}
	...
	def get_all_symbols(self):
		...
		for i in self.iter_symbols():
			...
			symbolList.append({"name" : symbol_name,"addr" : symbol_addr,"size" : symbol_size,"type" : symbol_type})
	...
	def get_all_segments(self):
		...
		for segment in self.elf.iter_segments():
			if segment.header.p_type == 'PT_LOAD':
				...
				segmentsList.append({"name":  name,"start":  Start ,"end":  (Start+size),"R":  R,"W":  W,"X":  E,"D": 0 ,"L": 0 ,"align": Align ,"base": 00 ,"type":  "","class":""  ,"ad": 32 ,"T": 00 ,"DS":  00})
	...
	def get_all_sections(self):
		...
		for segment in self.elf.iter_segments():
			if segment.header.p_type == 'PT_LOAD':
				...
				E = (segment.header.get("p_flags") >> 0) & 1
				Align = segment.header.get("p_align")
				segmentsMap[start] = {"start":  start ,"end":  (start+size),"R":  R,"W":  W,"X":  E,"D": 0 ,"L": 0 ,"align": Align ,"base": 00 ,"type":  "","class":""  ,"ad": 32 ,"T": 00 ,"DS":  00, "sections": []}
		...
		for i in segmentsMap:
			for j in segmentsMap[i].get("sections"):
				finalList.append({"name": j.name, "start":  hex(j.header.get("sh_addr")) ,"end":  "0xFFFFFFFF","R":  segmentsMap[i].get("R"),"W":  segmentsMap[i].get("W"),"X":  segmentsMap[i].get("X"),"D": 0 ,"L": 0 ,"align": segmentsMap[i].get("align") ,"base": 00 ,"type":  "","class":""  ,"ad": 32 ,"T": 00 ,"DS":  00})
